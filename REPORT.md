...
- 실시간 현재 날씨와 일별 예보를 함께 사용해 배경 판단 정확도 개선
- 사용자가 추가한 도시를 목록에서 삭제하는 기능
- 같은 이름의 검색 결과가 여러 개일 때 사용자가 지역을 선택하는 기능
- 에어코리아 측정소정보 API를 추가 신청해 검색 도시의 가장 가까운 측정소 자동 선택
- 대기질 데이터의 마지막 갱신 버튼과 재요청 기능
